...
export default function LeadershipSection() {
  ...
  const priorityLeaders = leaders.filter((leader) => /proprietor|headteacher|principal|director|administrator|deputy|coordinator|manager|head\s+of\s+department/i.test(leader.role || ''));
  const seniorLeaders = [...priorityLeaders, ...leaders.filter((leader) => !priorityLeaders.some((priority) => priority.id === leader.id))].slice(0, 4);
